...
var addr = flag.String("addr", "mhmeet.hive.clinic:1350", "https service address")
...
func main() {
	...
	osignal.Notify(interrupt, os.Interrupt)
...
//			wsclient.WriteMessage(websocket.CloseMessage, websocket.FormatCloseMessage(websocket.CloseNormalClosure, ""))
		...
		ICEServers: []webrtc.ICEServer{
			{
				...
				// URLs:           []string{"turn:139.185.42.143:3479"},
				URLs:           []string{"turn:193.123.93.151:3479"},
				Username:       "livebox",
				Credential:     "livebox123",
	...
	roomname := flag.Arg(3)
        ...
        Aport, err := strconv.Atoi(p1);
        Vport, err := strconv.Atoi(p2);


	udpConns := map[string]*udpConn{
		"audio": {port: Aport, payloadType: 111},
		"video": {port: Vport, payloadType: 96},
	}
	...
	//fmt.Println(signal.Encode(*peerConnection.LocalDescription()))
    	wsclient.WriteMessage(websocket.TextMessage, []byte(signal.Encode(*peerConnection.LocalDescription())+" "+peerid+" "+roomname))
